package com.bop.zz.photo.edit.utils;
...
public class FrameFilterUtils {
	 ...
	 * 		漏光效果
	 */
	public static int getLayerBlendingScreen(int baseColor, int overlayColor, double factor) {
		...
		r = (int) (255 - (255 - baseColor_r) * (255 - overlayColor_r * factor) / 255);
		g = (int) (255 - (255 - baseColor_g) * (255 - overlayColor_g * factor) / 255);
		b = (int) (255 - (255 - baseColor_b) * (255 - overlayColor_b * factor) / 255);
	...
	public static int getLayerBlendingMultiply(int baseColor, int overlayColor, double factor) {
	...
	public static int getLayerBlendingSoftLight(int colorA, int colorB) {
		...
		if (colorB_gray > 128) {
			r = (int) (colorA_r * (255 - colorB_r) / 128 + Math.sqrt(colorA_r / 255.0) * (2 * colorB_r - 255));
			g = (int) (colorA_g * (255 - colorB_g) / 128 + Math.sqrt(colorA_g / 255.0) * (2 * colorB_g - 255));
			b = (int) (colorA_b * (255 - colorB_b) / 128 + Math.sqrt(colorA_b / 255.0) * (2 * colorB_b - 255));

		} else {
			r = (int) (colorA_r * colorB_r / 128.0 + Math.pow(colorA_r / 255.0, 2) * (255 - 2 * colorB_r));
			g = (int) (colorA_g * colorB_g / 128.0 + Math.pow(colorA_g / 255.0, 2) * (255 - 2 * colorB_g));
			b = (int) (colorA_b * colorB_b / 128.0 + Math.pow(colorA_b / 255.0, 2) * (255 - 2 * colorB_b));
	...
	public static Bitmap getTestFilter(Bitmap base, Bitmap overlay) {
		...
		Bitmap tempBitmap = Bitmap.createBitmap(result, width, height, Config.ARGB_8888);
		return tempBitmap;
	}
}
