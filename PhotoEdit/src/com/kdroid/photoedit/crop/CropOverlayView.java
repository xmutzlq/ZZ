...
package com.kdroid.photoedit.crop;
...
public class CropOverlayView extends View {
	...
	/**
	 * Resets the crop overlay view. the Bitmap to set
	 * 
	 * @param
	 *
	 */
	...
	private void init(Context context) {
		...
		mCornerOffset = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, DEFAULT_CORNER_OFFSET_DP,
				displayMetrics);
		mCornerExtension = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, DEFAULT_CORNER_EXTENSION_DP,
				displayMetrics);
		mCornerLength = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, DEFAULT_CORNER_LENGTH_DP,
				displayMetrics);
	...
	private void initCropWindow(Rect bitmapRect) {
		...
		if (mFixAspectRatio) {
			...
			if (AspectRatioUtil.calculateAspectRatio(bitmapRect) > mTargetAspectRatio) {
				...
				final float cropWidth = Math.max(Edge.MIN_CROP_LENGTH_PX, AspectRatioUtil
						.calculateWidth(Edge.TOP.getCoordinate(), Edge.BOTTOM.getCoordinate(), mTargetAspectRatio));
				...
				// Limits the aspect ratio to no less than 40 wide or 40 tall
				final float cropHeight = Math.max(Edge.MIN_CROP_LENGTH_PX, AspectRatioUtil
						.calculateHeight(Edge.LEFT.getCoordinate(), Edge.RIGHT.getCoordinate(), mTargetAspectRatio));
	...
	private void drawCornerText(Canvas canvas) {
		...
		canvas.drawText(actualCropWidth + "x" + actualCropHeight,
				Edge.LEFT.getCoordinate() / 2 + +Edge.RIGHT.getCoordinate() / 2,
				Edge.TOP.getCoordinate() / 2 + Edge.BOTTOM.getCoordinate() / 2, mBorderPaint);
	...
	private void drawCorners(Canvas canvas) {
		...
		if (mCornerBitmap != null) {
			...
			// Top left
			canvas.drawLine(left - mCornerOffset, top - mCornerExtension, left - mCornerOffset, top + mCornerLength,
					mCornerPaint);
			canvas.drawLine(left, top - mCornerOffset, left + mCornerLength, top - mCornerOffset, mCornerPaint);

			// Top right
			canvas.drawLine(right + mCornerOffset, top - mCornerExtension, right + mCornerOffset, top + mCornerLength,
					mCornerPaint);
