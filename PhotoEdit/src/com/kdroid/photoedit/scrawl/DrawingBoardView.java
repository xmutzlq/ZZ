package com.kdroid.photoedit.scrawl;
...
public class DrawingBoardView extends View {
	...
	public void setBackgroundBitmap(Bitmap bitmap) {
		...
		paintBitmap = Bitmap.createBitmap(backgroundBitmap.getWidth(), backgroundBitmap.getHeight(),
	...
	class BrushGestureListener extends GestureDetector.SimpleOnGestureListener {
		...
		public boolean onDown(MotionEvent e) {
			...
			if (isStamp) {
				...
				paintCanvas.drawBitmap(brushBitmap, e.getX() - halfBrushBitmapWidth, e.getY() - halfBrushBitmapWidth,
						brushPaint);
			}
		...
		private void paintSingleStamp(float x, float y) {
			...
			for (int i = 1; i < stampBrushBitmaps.length; i++) {
				if (Math.random() > 0.3) {
					paintCanvas.drawBitmap(stampBrushBitmaps[i], x - halfBrushBitmapWidth, y - halfBrushBitmapWidth,
							null);
				}
			}
