package com.kdroid.photoedit.operate;
	...
	private void handleSingleTouchManipulateEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_MOVE:
			...
			if (mMovedSinceDown) {
				...
				int x = p.x + diffX;
				int y = p.y + diffY;
				if (p.x + diffX >= mCanvasLimits.left && p.x + diffX <= mCanvasLimits.right
						&& p.y + diffY >= mCanvasLimits.top && p.y + diffY <= mCanvasLimits.bottom)
